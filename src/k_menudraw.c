...
static void M_DrawCharSelectPreview(UINT8 num)
{
	...
	{
		if (p->mdepth == CSSTEP_NONE && num == setup_numplayers && gamestate == GS_TITLESCREEN)
		{
...
static void M_DrawReplayHutReplayInfo(menudemo_t *demoref)
{
	...
	case MD_OUTDATED:
		V_DrawThinString(17, 64, V_TRANSLUCENT|highlightflags, "Recorded on an outdated version.");
		/* FALLTHRU */
	default:
		// Draw level stuff
		x = 15; y = 15;
		...
		if (demoref->map < nummapheaders && mapheaderinfo[demoref->map])
		{
			...
			V_DrawString(x, y, V_TRANSLUCENT, "Level is not loaded.");

		if (demoref->numlaps)
			V_DrawThinString(x, y+9, 0, va("(%d laps)", demoref->numlaps));

		{
			...
			V_DrawString(x, y+20, 0, gtstring);
		}

		if (!demoref->standings[0].ranking)
		{
			// No standings were loaded!
			V_DrawString(x, y+39, V_TRANSLUCENT, "No standings available.");

			break;
		}

		V_DrawThinString(x, y+29, highlightflags, "WINNER");
		V_DrawString(x+38, y+30, 0, demoref->standings[0].name);
...
void M_DrawReplayHut(void)
{
	...
	for (i = 0; i < (INT16)sizedirmenu; i++)
	{
		...
		if (extrasmenu.demolist[i].type == MD_NOTLOADED && !processed_one_this_frame)
		{
			...
			G_LoadDemoInfo(&extrasmenu.demolist[i], false);
		}
		...
		if (itemOn == replaylistitem && i == (INT16)dir_on[menudepthleft])
		{
			...
			V_DrawString(localx - (extrasmenu.replayScrollTitle>>1), localy, highlightflags, extrasmenu.demolist[i].title);
		}
		else
			V_DrawString(localx, localy, 0, extrasmenu.demolist[i].title);
	}
...
#define STARTY 62-(extrasmenu.replayScrollTitle>>1)
	...
	for (i = 1; i < MAXPLAYERS && demoref->standings[i].ranking; i++)
	{
		...
		V_DrawThinString(BASEVIDWIDTH-96, STARTY + i*20, 0, demoref->standings[i].name);
...
#undef STARTY
	...
	V_DrawString(10, 72, highlightflags, demoref->title);
	...
	V_DrawSmallString(4, BASEVIDHEIGHT-14, 0, warning);
}
