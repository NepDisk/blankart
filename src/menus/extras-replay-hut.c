...
static void M_InitReplayHut(void)
{
	menuactive = true;
	G_SetGamestate(GS_MENU);
	titlemapinaction = false; // Nope don't give us HOMs please
}
...
void M_ReplayHut(INT32 choice)
{
	...
	{
		M_StartMessage("No replays", "No replays found.\n\nPress (B)\n", NULL, MM_NOTHING,NULL,NULL);
		...
		switch (dirmenu[dir_on[menudepthleft]][DIR_TYPE])
		{
			case EXT_FOLDER:
				...
				else
				{
					...
					M_StartMessage("Too Deep",va("%c%s\x80\nThis folder is too deep to navigate to!\n\nPress (B)\n", ('\x80' + (highlightflags>>V_CHARCOLORSHIFT)), M_AddonsHeaderPath()),NULL,MM_NOTHING,NULL,NULL);
					menupath[menupathindex[menudepthleft]] = 0;
				}
				break;
			case EXT_UP:
